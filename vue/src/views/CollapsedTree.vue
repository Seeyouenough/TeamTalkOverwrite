                     ...
                     <ul class="el-menu el-menu-vertical-demo collapsed" v-show="collapsed" ref="menuCollapsed">
                        <li v-for="(it,index) in child.children" v-if="!it.hidden" class="el-submenu item">
                             <template v-if="!it.leaf">
                               <div class="el-submenu__title" style="padding-left: 20px; marginLeft:200px" @mouseover="showMenu(index,it.name,true)" @mouseout="showMenu(index,it.name,false)"><i :class="it.iconCls"></i></div>
                               <ul class="el-menu submenu" :class="'submenu-hook-'+it.name+'-'+index" @mouseover="showMenu(index,it.name,true)" @mouseout="showMenu(index,it.name,false)"> 
                                 <li v-for="child_item in it.children" v-if="!child.hidden" :key="child.path" class="el-menu-item" style="padding-left: 40px; marginLeft:300px" :class="$route.path==child_item.path?'is-active':''" @click="$router.push(child_item.path)">{{child_item.name}}
